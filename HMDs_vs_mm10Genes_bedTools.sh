...
# create your working directory for your bedtools environment
    mkdir bed_tools_HMDvsGenes_mm10             # working directory should be outside the anaconda3 directory
    cd bed_tools_HMDvsGenes_mm10                # go to your bedtools working directory
...
#########################################################################################################################    
# HMD vs mm10 genes files to create inside_HMD_mm10_genes   
# HMD vs mm10 genes files # delete chrM containing lines
    bedtools intersect -a mm10_all_genes.bed -b HMD_mm10_modified.bed  > inside_HMD_mm10_genes_intersect.bed -wa

# remove duplicate rows from a file in Unix, you can use the uniq command
    sort inside_HMD_mm10_genes_intersect.bed | uniq > inside_HMD_mm10_genes_intersect_no_duplicates_not_sorted.bed

# if necassary, sort the file based on the first column
    sort -k1,1 inside_HMD_mm10_genes_intersect_no_duplicates_not_sorted.bed > inside_HMD_mm10_genes.bed

#########################################################################################################################

# HMD vs mm10 Enhancer files to create outside_HMD_mm10_genes
    bedtools intersect -a HMD_mm10_modified.bed -b mm10_all_genes.bed > outside_HMD_mm10_genes_intersect.bed -wa

# remove duplicate rows from a file in Unix, you can use the uniq command
    sort outside_HMD_mm10_genes_intersect.bed | uniq > outside_HMD_mm10_genes_intersect_no_duplicates_not_sorted.bed

# sort the file based on the first column
    sort -k1,1 outside_HMD_mm10_genes_intersect_no_duplicates_not_sorted.bed > outside_HMD_mm10_genes_no_duplicates_sorted.bed

# Reporting the absence of any overlapping features
    bedtools intersect -a mm10_all_genes.bed -b outside_HMD_mm10_genes_no_duplicates_sorted.bed > outside_HMD_mm10_genes.bed -v
