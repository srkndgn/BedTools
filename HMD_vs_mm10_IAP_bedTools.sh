...
# create your working directory for your bedtools environment
    mkdir bed_tools_HMDvsIAP             # working directory should be outside the anaconda3 directory
    cd bed_tools_HMDvsIAP                # go to your bedtools working directory
...
# HMD vs mm10 Enhancer files to create inside_HMD_mm10_Enhancers
    bedtools intersect -a mm10_LTR_IAP_modified.bed -b HMD_mm10_modified.bed > inside_HMD_mm10_IAP_intersect.bed -wa

# remove duplicate rows from a file in Unix, you can use the uniq command
    sort inside_HMD_mm10_IAP_intersect.bed | uniq > inside_HMD_mm10_IAP_no_duplicates_not_sorted.bed

# sort the file based on the first column
    sort -k1,1 inside_HMD_mm10_IAP_no_duplicates_not_sorted.bed > inside_HMD_mm10_IAP.bed

#########################################################################################################################

# HMD vs mm10 Enhancer files to create outside_HMD_mm10_Enhancers
    bedtools intersect -a HMD_mm10_modified.bed -b mm10_LTR_IAP_modified.bed > outside_HMD_mm10_IAP_intersect.bed -wa

# remove duplicate rows from a file in Unix, you can use the uniq command
    sort outside_HMD_mm10_IAP_intersect.bed | uniq > outside_HMD_mm10_IAP_no_duplicates_not_sorted.bed

# sort the file based on the first column
    sort -k1,1 outside_HMD_mm10_IAP_no_duplicates_not_sorted.bed > outside_HMD_mm10_IAP_no_duplicates_sorted.bed

# Reporting the absence of any overlapping features
    bedtools intersect -a mm10_LTR_IAP_modified.bed -b outside_HMD_mm10_IAP_no_duplicates_sorted.bed > outside_HMD_mm10_IAP.bed -v
